function [DM, DM_pval] = dmtest(e1, e2, h, side)
...
%   side = 1 for one-sided to the right, two-sided test is the default (write 0 for eg.)
%
...
if side == 1
    DM_pval     = 1-normcdf(DM); % one-sided to the right
else 
    DM_pval     = 2*(1-normcdf(abs(DM))); %two sided test
end
